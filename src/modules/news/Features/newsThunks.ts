...
export const getNewsThunk = createAsyncThunk<
  ...
  void,
  { rejectValue: string }
>("/news", async (news: void, thunkAPI) => {
  try {
    const response = await fetch(`${import.meta.env.VITE_API_URL}/news`);
    if (!response.ok) {
      const errorNews = await response.json();
      return thunkAPI.rejectWithValue(
        errorNews.error ?? "Error al obetener todas las noticias",
      );
    }
    const data: AllNewsInterface = await response.json();
    return data;
  } catch (error) {
    return thunkAPI.rejectWithValue("Error al obetener todas las noticias");
  }
});
...
try {
 ...
 if (!response.ok) {
    ...
    return thunkAPI.rejectWithValue("Error al eliminar laa noticia");
}
});
